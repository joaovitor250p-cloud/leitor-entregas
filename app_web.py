...
# DICIONÁRIO MULTILÍNGUE (PT / ES / EN)
DICIONARIO = {
    "Português 🇧🇷": {
        "lang_code": "pt-BR",
...
with st.sidebar:
    ...
    txt = DICIONARIO[idioma_selecionado]
    st.caption(txt["subtitulo"])
    st.write("---")
    
    modo_cor_opcao = st.selectbox(
        txt["cor_modo"],
        [txt["opcao_preto"], txt["opcao_branco"]],
        index=0
    )
    tema_escuro = (modo_cor_opcao == txt["opcao_preto"])
    
    usar_audio = st.toggle(txt["falar_toggle"], value=True)
    tipo_voz_index = 0
    if usar_audio:
        ...
        tipo_voz_index = txt["vozes"].index(tipo_voz)
        
    st.write("---")
    if st.button(txt["btn_zerar"]):
        st.session_state.pacotes_bipados = set()
        st.rerun()
...
st.markdown(
    ...
    '<div class="welcome-subtitle">' + txt["subtitulo"] + '</div>'
    '</div>',
    unsafe_allow_html=True
)
...
st.markdown(
    ...
    '<div class="upload-title">' + txt["upload_titulo"] + '</div>'
    '<div class="upload-sub">' + txt["upload_sub"] + '</div>'
...
if not arquivo_pdf:
    st.markdown(
        '<div class="pix-card">'
        '    <div class="pix-title">' + txt["pix_titulo"] + '</div>'
        '    <div class="pix-desc">' + txt["pix_desc"] + '</div>'
        '    <div class="pix-key">' + txt["pix_rotulo"] + ' ' + CHAVE_PIX + '</div>'
        '</div>',
        unsafe_allow_html=True
    )
...
if arquivo_pdf:
    ...
    with st.expander(txt["duplos_titulo"]):
        ...
        if not encontrou_duplo:
            st.info(txt["nenhum_duplo"])

    st.markdown(
        '<div class="camera-header">'
        '<div class="camera-title">' + txt["camera_titulo"] + '</div>'
        '<div class="camera-sub">' + txt["camera_sub"] + '</div>'
        '</div>',
        unsafe_allow_html=True
    )
